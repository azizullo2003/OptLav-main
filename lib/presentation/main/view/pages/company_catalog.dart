...
class CompanyCatalog extends StatefulWidget {
  ...
  const CompanyCatalog({super.key, required this.company, this.fromFavorite});
...
class _CompanyCatalogState extends State<CompanyCatalog> {
  ...
  void _sendActivity(BuildContext context) {
    BlocProvider.of<SendActivityBloc>(context).add(
        SendActivityEvent.sendActivity(
            screenName: "Экран каталога компании ${widget.company.name}"));
  }
  ...
  void _onProduct(Company company, String categoryId, String categoryName) {
    if (widget.fromFavorite != null) {
      ...
      context.router.replace(CompanyProducts(
          company: company,
          categoryId: categoryId,
          categoryName: categoryName));
    }
